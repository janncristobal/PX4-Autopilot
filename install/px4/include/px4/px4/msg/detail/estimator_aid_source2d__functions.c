  ...
  px4__msg__EstimatorAidSource2d__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    px4__msg__EstimatorAidSource2d * data =
      (px4__msg__EstimatorAidSource2d *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!px4__msg__EstimatorAidSource2d__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          px4__msg__EstimatorAidSource2d__fini(&output->data[i]);
        }
        return false;
      }
    }
